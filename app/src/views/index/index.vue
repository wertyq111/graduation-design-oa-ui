    ...
    <loader :loading="loading">
      ...
      <template slot="body">
        ...
        <el-image style="animation: header-effect 2s"
                  ...
                  v-once
                  lazy
                  :src="!$common.isEmpty(web.webInfo.backgroundImage)?web.webInfo.backgroundImage:$constant.random_image+new Date()+Math.floor(Math.random()*10)"
                  fit="cover">
        ...
        <div class="signature-wall myCenter my-animation-hideToShow">
          <h1 class="playful">
            <span v-for="(a, index) in web.webInfo.webTitle" :key="index">{{ a }}</span>
          </h1>
          ...
          <div class="page-container">
            ...
            <div class="recent-posts">
              <div class="announcement background-opacity">
                ...
                <div>
                  <div v-for="(notice, index) in web.webInfo.notices" :key="index">
                    {{ notice }}
                  </div>
                </div>
              </div>
...
import WebFooter from '@/layout/web-footer.vue';
...
export default {
  ...
  methods: {
    ...
    async getArticles() {
      await this.$http.post("/api/articles", this.pagination)
        .then((res) => {
