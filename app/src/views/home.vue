    ...
    <transition name="el-fade-in-linear">
      ...
      <div v-show="toolbar.visible"
        ...
        <div class="toolbar-title">
          <h2 @click="$router.push({path: '/'})">{{ webInfo.webName }}</h2>
        </div>
            ...
            <li>
              <el-dropdown placement="bottom">
                <el-avatar class="user-avatar" :size="36"
                           ...
                           :src="!$common.isEmpty($store.state.web.currentUser)?$store.state.web.currentUser.avatar:$store.state.web.webInfo.avatar">
                </el-avatar>

                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item @click.native="$router.push({path: '/user'})"
                                    v-if="!$common.isEmpty($store.state.web.currentUser)">
                    <i class="fa fa-user-circle" aria-hidden="true"></i> <span>个人中心</span>
                  </el-dropdown-item>
                  <el-dropdown-item @click.native="logout()" v-if="!$common.isEmpty($store.state.web.currentUser)">
    ...
    <el-drawer :visible.sync="toolbarDrawer"
      ...
      <div>
        <ul class="small-menu">
          ...
          <template v-if="$common.isEmpty($store.state.web.currentUser)">
...
export default {
  ...
  watch: {
    scrollTop(scrollTop, oldScrollTop) {
      ...
      this.$store.dispatch("web/changeToolbarStatus", toolbarStatus);
    },
  },
  created() {
    let toolbarStatus = {
  ...
  computed: {
    toolbar() {
      return this.$store.state.web.toolbar;
    },
    webInfo() {
  ...
  methods: {
    ...
    goIm() {
      if (this.$common.isEmpty(this.$store.state.web.currentUser)) {
        this.$message({
          message: "请先登录！",
          type: "error"
        });
    ...
    logout() {
      ...
      this.$store.dispatch("web/loadCurrentUser", {});
      localStorage.removeItem("userToken");
      this.$router.push({path: '/'});
    },
    getWebInfo() {
      this.$http.get("/api/web-info")
        .then((res) => {
          if (!this.$common.isEmpty(res.data)) {
            this.$store.dispatch("web/loadWebInfo", res.data);
          }
        })
        .catch((error) => {
          this.$message({
    ...
    getCategories() {
      this.$http.get("/api/web-categories").then((res) => {
        if (!this.$common.isEmpty(res.data)) {
          this.$store.dispatch("web/loadCategories", res.data);
        }
      })
        .catch((error) => {
          this.$message({
